...
def hyperparameter_search(models, dataset, device):
    # Example grid of hyperparameters
    param_grid = [
        {'embed_dim': 128, 'lr': 1e-3},
        {'embed_dim': 128, 'lr': 1e-4},
        {'embed_dim': 256, 'lr': 1e-3},
        {'embed_dim': 256, 'lr': 1e-4},
    ]
    ...
    for model_name, model_class in models.items():
        for params in param_grid:
            # Create a new instance of the model with chosen embed_dim
            input_dim = dataset[0][0].shape[1] * dataset[0][0].shape[0]
            model = model_class(input_dim, params['embed_dim']).to(device)
            teacher_model = model_class(input_dim, params['embed_dim']).to(device)
            ...
            # Short training loop
            for _ in range(3):
                # ...existing code (train_epoch, validate)...
                _train_loss, _train_acc = train_epoch(model, teacher_model, train_loader, optimizer, device, criterion)
                _val_loss, _val_acc = validate(model, val_loader, criterion, device)

            if _val_acc > best_score:
                best_score = _val_acc
                best_params = (model_name, params.copy())
...
if __name__ == "__main__":
    ...
    score, params = hyperparameter_search(models, dataset, device)
    print(f"Best score: {score:.4f} with config: {params}")